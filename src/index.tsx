...
} from "@photo-sphere-viewer/resolution-plugin";
import {
  SettingsPlugin,
  SettingsPluginConfig,
} from "@photo-sphere-viewer/settings-plugin";
import { StereoPlugin } from "@photo-sphere-viewer/stereo-plugin";
import {
  VideoPlugin,
  VideoPluginConfig,
} from "@photo-sphere-viewer/video-plugin";
import {
  VirtualTourPlugin,
...
} from "@photo-sphere-viewer/visible-range-plugin";
...
} from "@photo-sphere-viewer/cubemap-video-adapter";
import {
...
const ReactPhotoSphereViewer = forwardRef(
  (options: Props, ref: unknown): React.ReactElement => {
    ...
    const [currentNavbar, setCurrentNavbar] =
      useState<(string | object)[]>(defaultNavbar);

    useEffect(() => {
      function handleResize() {
        const aspectRatio = window.innerWidth / window.innerHeight;
        //console.log(aspectRatio)
        LITTLEPLANET_MAX_ZOOM = Math.floor(
          map(aspectRatio, 0.5, 1.8, 140, 115)
        );
      }
      // Add event listener
    ...
    useEffect(() => {
      ...
      if (sphereElementRef.current && !spherePlayerInstance.current) {
        const _c = new Viewer({
          ...
          fisheye: options.littlePlanet
            ? LITTLEPLANET_FISHEYE
            : options.fisheye || false,
          minFov: options.minFov || 30,
          maxFov: options.littlePlanet
            ? LITTLEPLANET_MAX_ZOOM
            : options.maxFov || 90,
          defaultZoomLvl: options.littlePlanet
            ? LITTLEPLANET_DEF_ZOOM
            : options.defaultZoomLvl || 50,
          defaultYaw: options.defaultYaw || 0,
          defaultPitch: options.littlePlanet
            ? LITTLEPLANET_DEF_LAT
            : options.defaultPitch || 0,
          ...
          plugins: [
            ...
            ...(options.plugins
              ? (options.plugins as PluginConstructor[])
              : []),
          ],
        });
        _c.addEventListener(
        ...
        if (options.littlePlanet) {
          ...
          if (
            ...
            !_currentNavbar.find(
              (item) =>
                typeof item === "object" &&
                item?.id === "resetLittlePlanetButton"
            )
          ) {
